...
## Bug Fix
- accept_join_request now fetches max_team_size from the hackathon (not the team, which has no such column)
...
-- Atomically: marks request accepted, adds team member, notifies sender, enforces capacity.
...
BEGIN
  ...
  IF v_request.team_id IS NOT NULL THEN
    ...
    SELECT * INTO v_hackathon FROM hackathons WHERE id = v_team.hackathon_id;
